...
import com.application.flightweathertracker.imgw.taf.database.long_term.LongTafResponses;
    ...
    public void saveImgwTafResponse() {
        ...
        for (TafData tafData : deserializedTafResponse.values()) {
            if (tafData.getFc() != null) {
                if (!shortTafResponsesRepository.existsByStationAndObservedAt(tafData.getFc().station(), tafData.getFc().date())) {
                    try {
                        ShortTafResponses responseRecord = ShortTafResponses.builder()
            ...
            if (tafData.getFt() != null) {
                if (!longTafResponsesRepository.existsByStationAndObservedAt(tafData.getFt().station(), tafData.getFt().date())) {
                    try {
                        LongTafResponses responseRecord = LongTafResponses.builder()
